...
- **`cardLimitUsage(cardId)` é diferente de `cardInvoiceTotals`**: os números Lançado/Pendente/Previsto exibidos continuam só do mês em questão (não mexer nisso), mas a barra "Limite do cartão" soma `Lançado`/`Parcial` de **todos os meses** (já foi gasto, só não foi pago, mesmo que a fatura seja futura) + `Pendente` só do mês atual de verdade (`todayKey()`, não `currentMonth` — o usuário pode estar navegando outro mês no "Mensal"). Existe porque uma compra já lançada numa fatura futura já consome limite de crédito real, não só a fatura do mês corrente.
...
- **Painel "Crédito disponível" na Visão Geral** (`renderCardsSummary()`) é um resumo consolidado (soma de todos os limites vs. `cardLimitUsage()` de todos os cartões), diferente do painel "Faturas dos cartões" do "Mensal" — decisão explícita pra não duplicar a mesma tabela por cartão nas duas telas.
